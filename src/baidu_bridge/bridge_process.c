...
int do_expire_event(struct redisClient *c,sds *funcname)
{
...
{
    struct dictIterator *iter=dictGetIterator(c->db->bridge_db.triggle_scipts);
    dictEntry *trigs;
    do{
        trigs=dictNext(iter);
        if(trigs!=NULL)
            ...
            if(tmptrg->event==event_type){ //找到指定的类型事件
                ...
                switch(event_type){
                    case DELETE_EVENT: 
                        ...
                        break;
                    case EXPIRED_EVENT:
                         do_expire_event(c,dictGetKey(trigs));
                            break;
                    default:
